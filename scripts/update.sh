...
sudo -u "$APP_USER" bash -c "git fetch origin"
sudo -u "$APP_USER" bash -c "git reset --hard origin/main"

# Install any new dependencies using venv pip directly (no activate needed)
sudo -u "$APP_USER" bash -c "\"$PROJECT_DIR/venv/bin/pip\" install -r requirements.txt"
